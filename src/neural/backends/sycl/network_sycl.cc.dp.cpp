...
class SyclNetwork : public Network {
 public:
  SyclNetwork(const WeightsFile& file, const OptionsDict& options)
    ...
    sycl_queue_ =
        std::make_unique<sycl::queue>(context, devices[gpu_id_], exceptions_handler,
    ...
    if (!multi_stream_) {
      for (int i = 0; i < 3; i++) {
        tensor_mem_[i] = (DataType*)sycl::malloc_device(maxSize, *sycl_queue_);
        tensor_mem_owner_[i] = DeviceMem(tensor_mem_[i], sycl_queue_.get());
        sycl_queue_->memset(tensor_mem_[i], 0, maxSize);
      }
      sycl_queue_->wait();
    }
  ...
  void forwardEval(InputsOutputs* io, int batchSize) {
    ...
    CERR << "TRACE: Before expandPlanes\n";
    if (fp16) {
    ...
    if (numBlocks_ > 0) {
      // Input.
      CERR << "TRACE: Before input conv Eval\n";
      io_sycl_queue_.wait();
      ...
      for (int block = 0; block < numBlocks_; block++) {
        ...
        io_sycl_queue_.wait();
        CERR << "TRACE: After Residual block " << block << "\n";
      }
    ...
    if (attn_body_) {
      CERR << "TRACE: Before Attention Body\n";
      io_sycl_queue_.wait();
      network_[l++]->Eval(
    ...
    if (moves_left_) {
      CERR << "TRACE: Before Moves Left Head\n";
      io_sycl_queue_.wait();
      // Moves left head
      network_[l++]->Eval(batchSize, spare1, flow, nullptr, scratch_mem,
                          scratch_size_, io_sycl_queue_,
    ...
    CERR << "TRACE: Before copying policy output to host\n";
    io_sycl_queue_.wait();
#ifndef USE_INTEL
    auto event =
 ...
 private:
  ...
  BaseLayer<DataType>* resi_last_ = nullptr;
  ...
  struct DeviceMem {
    ...
    DeviceMem() = default;
    DeviceMem(void* p, sycl::queue* queue) : ptr(p), q(queue) {}
    ~DeviceMem() { if (ptr && q) sycl::free(ptr, *q); }
    DeviceMem(const DeviceMem&) = delete;
    DeviceMem& operator=(const DeviceMem&) = delete;
    DeviceMem(DeviceMem&& other) : ptr(other.ptr), q(other.q) { other.ptr = nullptr; }
  ...
  DeviceMem tensor_mem_owner_[3];
  DataType* tensor_mem_[3] = {nullptr, nullptr, nullptr};
