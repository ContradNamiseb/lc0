...
//
// One thread group per (batch, head); BLOCK_SIZE threads per group, one per
// value-dimension lane. Hardcoded to this network's key_dim = value_dim = 32
// (see docs/kda_split.textproto's kda { key_dim: 32, value_dim: 32 }) --
// HLSL's numthreads must be a compile-time constant, so supporting other
// sizes would need the multi-variant-file approach dx/shaders/SE.hlsl uses
// for its channel counts. Not done here for lack of a second configuration
// that would need it.
...
cbuffer KdaRecurrenceConstants : register(b0) {
  ...
  uint key_dim;            // <= BLOCK_SIZE, checked on the C++ side
  uint value_dim;          // <= BLOCK_SIZE, checked on the C++ side
...
groupshared float p_q[BLOCK_SIZE];
groupshared float p_k[BLOCK_SIZE];
groupshared float p_decay[BLOCK_SIZE];

[numthreads(BLOCK_SIZE, 1, 1)]
void KdaRecurrence(uint3 gid : SV_GroupID, uint3 tid : SV_GroupThreadID) {
  // No early return for local_id >= value_dim (BLOCK_SIZE=32 may exceed a
  // smaller runtime value_dim): the two GroupMemoryBarrierWithGroupSync()
  // calls below must be reached by every thread in the group uniformly --
  // an early return before them is illegal divergent flow control (HLSL
  // error X4026) even though SYCL's identical barrier never had this
  // problem, since SYCL dispatches exactly value_dim work-items and never
  // has idle lanes to guard against. Instead, the output write near the
  // bottom of the loop is what's gated on local_id < value_dim; idle lanes
  // still participate in the staging loop below (redundantly recomputing
  // -- not reading garbage from -- p_q/p_k/p_decay entries another lane
  // already wrote, which is a benign race in practice on this hardware but
  // not one BLOCK_SIZE == value_dim configurations, like this network's,
  // ever exercise).
  ...
  const float scale = 1.0 / sqrt((float)key_dim);
  const float decay_scale = exp((float)a_log[head]);
  const uint key_depth = heads * key_dim;
  const uint value_depth = heads * value_dim;
  ...
  for (uint token = 0; token < 64; ++token) {
    ...
    if (use_fused_qkv != 0) {
      q_off = token_idx * qkv_stride + head * key_dim;
      k_off = token_idx * qkv_stride + key_depth + head * key_dim;
      v_off = token_idx * qkv_stride + 2 * key_depth + head * value_dim;
    } else {
      q_off = token_idx * key_depth + head * key_dim;
      k_off = token_idx * key_depth + head * key_dim;
      v_off = token_idx * value_depth + head * value_dim;
    }

    const uint raw_decay_offset = token_idx * key_depth + head * key_dim;
    const uint value_offset = token_idx * value_depth + head * value_dim;

    // Strided write into groupshared memory: mirrors the SYCL kernel's
    // "local_id < value_dim lanes covering key_dim entries" loop.
    for (uint i = local_id; i < key_dim; i += value_dim) {
      ...
      const float decay_input =
          (float)raw_decay[raw_decay_offset + i] + (float)dt_bias[head * key_dim + i];
    ...
    for (uint key2 = 0; key2 < key_dim; ++key2) {
    ...
    for (uint key3 = 0; key3 < key_dim; ++key3) {
      prediction += p_k[key3] * k_norm * state[key3];
    }

    // Gated, not the whole function: an idle lane (local_id >= value_dim,
    // only possible when BLOCK_SIZE=32 > this call's runtime value_dim)
    // must not read v[v_off + local_id] -- out of range for this token's
    // value slice -- or write mixed[value_offset + local_id], which would
    // land in the next head's or token's region. Every lane still computed
    // prediction above (harmless, private per-thread state), just doesn't
    // act on it here.
    if (local_id < value_dim) {
      const float v_value =
          (float)(use_fused_qkv != 0 ? qkv[v_off + local_id] : v_in[v_off + local_id]);
      const float delta = update_rate * (v_value - prediction);

      float output = 0.0;
      for (uint key4 = 0; key4 < key_dim; ++key4) {
    ...
    // by every thread above but only rewritten by the first key_dim threads
    // at the top of the next iteration -- without this, a fast thread can
